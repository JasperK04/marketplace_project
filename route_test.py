...
def test_modify_user(
    *, expected, token, name=None, username=None, email=None, password=None
):
    """Test the user creation route."""
    data = {}
    if name:
    ...
    assert data, "No new data is specified"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.patch(f"{BASE_URL}/users", json=data, headers=headers)
    assert response.status_code == expected, (
        f"Unexpected status code: {response.status_code}"
    )
    return response.json().get("id")
...
def test_get_token(username, password, *, expected):
    ...
    response = requests.post(f"{BASE_URL}/tokens", auth=auth)
    assert response.status_code == expected, (
        f"Unexpected status code: {response.status_code}"
    )
...
def test_create_listing(
    *, title, price, description, token, expected, categoryID=None, category=None
):
...
def test_delete_listing(id, *, token, expected):
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.delete(f"{BASE_URL}/listings/{id}", headers=headers)
    assert response.status_code == expected, (
        f"Unexpected status code: {response.status_code}"
    )
...
if __name__ == "__main__":
    ...
    test_create_user(
        name, username, "invalid-email", password, expected=400
    )  # invalid email
    test_create_user(
        name + "漢字", username, fake.email(), password, expected=400
    )  # invalid name
    test_create_user(
        name + "2", username + "漢字", email, password, expected=400
    )  # invalid username
    test_create_user(
        name + "3", username, fake.email(), fake.password(10), expected=400
    )  # invalid password
    ...
    assert id == test_modify_user(
        expected=200, token=token, name=fake.name()
    )  # modify name
    assert id == test_modify_user(
        expected=200, token=token, username=fake.user_name()
    )  # modify name
    assert id == test_modify_user(
        expected=200, token=token, email=fake.email()
    )  # modify email
    assert id == test_modify_user(
        expected=200, token=token, password=fake.password(32)
    )  # modify password
    ...
    test_create_listing(
        ...
        token=token,
        expected=400,
    )  # fake category (invalid listing)
    test_get_listing(listing_id, expected=200)
    test_get_listings(expected=200)

    assert listing_id == test_modify_listing(
        expected=200, token=token, id=listing_id, title=fake.word()
    )
    assert listing_id == test_modify_listing(
        expected=200, token=token, id=listing_id, price="123456"
    )
    assert listing_id == test_modify_listing(
        expected=200, token=token, id=listing_id, description=fake.sentence()
    )
    assert listing_id == test_modify_listing(
        expected=200, token=token, id=listing_id, categoryID=1
    )
    test_modify_listing(
        expected=400, token=token, id=listing_id, category="another fake category"
    )
    ...
    test_buy_listing(
        listing_id, token=token2, expected=400
    )  # can't buy an already bought listing
    ...
    listing_id = test_create_listing(
        ...
        token=token,
        expected=201,
    )  # new listing to check if its deleted
