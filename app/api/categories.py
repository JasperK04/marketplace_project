...
def get_categories():
    ...
    return Category.to_collection_dict(
        sa.select(Category), page, per_page, "api.get_listings"
    )
...
def create_category():
    ...
    if db.session.scalar(
        sa.select(Category).where(  # check the category name does not yet exist
    ...
    db.session.add(cat)
    db.session.commit()
    return cat.to_dict(), 201, {"Location": url_for("api.get_category", id=cat.id)}
